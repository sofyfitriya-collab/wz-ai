...
async function askWzAI(){
 ...
 try{
  ...
  box.innerHTML=`<div style="white-space:pre-wrap;line-height:1.7">${ownerChartEsc(r.answer||'Tidak ada jawaban.')}</div><div class="subtle" style="margin-top:10px">Sumber: ${ownerChartEsc(r.source||'WZ DATA')} · Scope: ${ownerChartEsc(branch==='ALL'?'Semua Cabang':branch)}${r.aiConfigured?'':' · AI provider belum dikonfigurasi, mode jawaban data dasar digunakan.'}</div>`;
